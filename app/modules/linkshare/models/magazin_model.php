...
*/
	...
	*/
	function get_magazine ($filters) {
		$row = array();						
                ...
                if(isset($filters['id_site'])) $this->db->where('id_site',$filters['id_site']);
                if(isset($filters['id_status'])) $this->db->where('id_status',$filters['id_status']);
                $this->db->order_by('id');
		$result = $this->db->get('linkshare_magazin');
                if(isset($filters['name']))   $this->load->model('site_model');
	...
	*/
	function get_magazin ($id,$name=FALSE) {
		...
		$result = $this->db->get('linkshare_magazin');
                ...
                if($name)   $this->load->model('site_model');
	...
	*/
	function get_count_produse_by_mid ($mid,$id_site,$filters = array()) {
		...
		$this->db->where('mid',$mid);
                $this->db->where('id_site',$id_site);
                if(isset($filters['cat_creative_id']))    $this->db->where('cat_creative_id',$filters['cat_creative_id']);
                if(isset($filters['cat_creative_name']))    $this->db->where('cat_creative_name',$filters['cat_creative_name']);
	...
	* @return int $count
	*/
	function new_magazine ($mids) {                                                                                                                                                           $cate = count($mids);
           for($i=0;$i<$cate;$i++) {
	...
	*/
	function delete_magazin ($id) {																										
        ...
        function parse_magazin($params) {            
            if($params[0]['limit']) $limit = $params[0]['limit']; else $limit = 10;
            if($params[0]['offset'])    $offset = $params[0]['offset']; else $offset = 0;
