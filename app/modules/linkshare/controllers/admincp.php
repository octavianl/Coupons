...
class Admincp extends Admincp_Controller {
	...
	function index () {		
		...
		$columns = array(
                ...
                if(isset($_GET['offset'])) $filters['offset'] = $_GET['offset'];
                if(isset($_GET['cat_creative_id'])) $filters['cat_creative_id'] = $_GET['cat_creative_id'];
                
                if(isset($_GET['filters'])) {
                    $aux = unserialize(base64_decode($this->asciihex->HexToAscii($_GET['filters'])));
                    if(isset($aux['cat_creative_id'])) $filters['cat_creative_id'] = $aux['cat_creative_id'];
                    if(isset($aux['cat_creative_name'])) $filters['cat_creative_name'] = $aux['cat_creative_name'];
                }
   ...
   function add_status () {
		...
		$columns = array(
            ...
            $filters['id_site'] = $id;            
            $filters['name'] = TRUE;
            if(isset($_GET['nume'])) $filters['nume'] = $_GET['nume'];
            ...
            if(isset($_GET['filters'])) {
                $aux = unserialize(base64_decode($this->asciihex->HexToAscii($_GET['filters'])));
                if(isset($aux['nume'])) $filters['nume'] = $aux['nume'];
            }
                ...
                if(is_object($categories) && isset($categories)){
                    ...
                    if($kids){
                        ...
                        {               
                            ...
                            if(!$exista)  {  $this->produs_model->new_produs ($product[$i]); $j++; }
                            else { $this->produs_model->update_produs_by_linkid($product[$i],$product[$i]['linkid']); $k++; }
                            $i++;
                            //echo $i.'<br/>';
                ...
                if(!$xml->open($file)){
                    ...
                    if(!empty($produs_partial)) {                    
                        ...
                        if(is_object($produs) && isset($produs)){
                            ...
                            if(!$exista)  {  $this->produs_model->new_produs ($produs_partial); $j++; }
                            else { $this->produs_model->update_produs_by_linkid($produs_partial,$produs_partial['linkid']); $k++; }
